			...
			if (results.length === 0) {
				db.query(
					...
					(err, insertResult) => {
						...
						db.query(
							'INSERT INTO student_requests (student_id, teacher_id) VALUES (?, ?)',
							[studentInsertId, teacherId],
							(err, insertRequestResult) => {
								if (err) {
									console.error(err)
									return res.status(500).json({ message: 'Ошибка сервера' })
								}
				...
				return res
					.status(210)
					.json({ message: 'Запрос от данного студента уже существует' })
			}
		}
	)
})
...
app.get('/admin/requests', (req, res) => {
	db.query(
		'SELECT id, (SELECT student_id FROM students WHERE students.id = student_requests.student_id) AS student_id, teacher_id FROM student_requests',
		(err, results) => {
			if (err) {
				console.error(err)
				return res.status(500).json({ message: 'Ошибка сервера' })
...
app.get('/lectures', (req, res) => {
	db.query(
		'SELECT id, title, description, difficulty FROM lectures',
		(err, results) => {
			if (err) {
				console.error(err)
				return res.status(500).json({ message: 'Ошибка сервера' })
...
app.post('/addLecture', (req, res) => {
	...
	if (!title || !description || !difficulty) {
		return res
			.status(400)
			.json({ message: 'Введите название, описание и сложность лекции' })
	}
	...
	db.query(
		'INSERT INTO lectures (title, description, difficulty, current_id) VALUES (?, ?, ?, ?)',
		[title, description, difficulty, null],
		(err, insertResult) => {
			if (err) {
				console.error(err)
				return res.status(500).json({ message: 'Ошибка сервера' })
			...
			db.query(
				'UPDATE lectures SET current_id = ? WHERE id = ?',
				[lectureId, lectureId],
				(err, updateResult) => {
					if (err) {
						console.error(err)
...
app.get('/lectures/:lectureId', (req, res) => {
	...
	if (!lectureId) {
		return res
			.status(400)
			.json({ message: 'Укажите корректный идентификатор лекции' })
	}

	// Запрашиваем информацию о лекции из базы данных по идентификатору
	db.query(
		'SELECT id, title, description, difficulty FROM lectures WHERE id = ?',
		[lectureId],
		(err, results) => {
			if (err) {
				console.error(err)
...
io.on('connection', socket => {
	socket.on('join', data => {
		...
		db.query('SELECT * FROM licenses WHERE uid = ?', [uid], (err, results) => {
			...
			if (
				results.length === 0 ||
				!results[0].is_active ||
				new Date(results[0].expiration_date) < new Date()
			) {
				socket.emit('invalid-uid-license', {
					uid,
					message: 'UID or license is invalid',
				})
				return
			}
			socket.join(uid)
	...
	socket.on('subject-and-class', ({ uid: targetUid, subject, grade }) => {
		...
		if (socket.uid !== targetUid) {
			io.to(targetUid).emit('selected-subject-and-class', {
				uid: targetUid,
				subject,
				grade,
			})
	...
	socket.on('restart-timer', targetUid => {
		...
		if (!timerStopped) {
			io.to(targetUid).emit('time-received', {
				uid: targetUid,
				timeInSeconds: clients[targetUid].timeInSeconds,
			})
		}
	})
	...
	db.query(
		...
		(err, results) => {
			if (err) {
				...
				return res
					.status(500)
					.json({ error: 'Ошибка регистрации пользователя' })
			}
			return res
				.status(200)
	...
	db.query(
		...
		(err, results) => {
			if (err) {
				...
				return res
					.status(500)
					.json({ error: 'Ошибка аутентификации пользователя' })
			}
			...
			return res
				.status(200)
				.json({ message: 'Пользователь успешно вошел в систему', userId })
		}
	)
})
...
app.post('/purchaseLicense', (req, res) => {
	...
	const licenseExpirationDate = currentDate
		.clone()
		.add(selectedPlan.days, 'days')
	...
	db.query('SELECT email FROM users WHERE id = ?', [userId], (err, results) => {
		if (err) {
			...
			return res
				.status(500)
				.json({ error: 'Ошибка получения email пользователя' })
		}
		...
		db.query(
          ...
          `,
				...
				return res
					.status(200)
					.json({ message: 'Лицензия успешно создана', uid })
			}
		)
	})
})
...
app.post('/renewLicense', (req, res) => {
	...
	db.query(
		...
		(err, results) => {
			...
			const newExpirationDate = currentExpirationDate
				.clone()
				.add(licenseDays, 'days')
...
app.get('/licenseStatus/:userId', (req, res) => {
	...
	db.query(
		'SELECT * FROM licenses WHERE user_id = ? AND is_active = true',
		userId,
		(err, results) => {
			if (err) {
				console.error(err)
				return res
					.status(500)
					.json({ error: 'Ошибк при проверке статуса лицензии' })
			}

			if (results.length === 0) {
				return res
					.status(404)
					.json({ active: false, message: 'Лицензия не активна' })
			}
...
app.get('/licenseInfo/:userId', (req, res) => {
	...
	db.query(
		'SELECT uid, DATE_FORMAT(expiration_date, "%Y-%m-%dT%H:%i:%s.000Z") as expiration_date FROM licenses WHERE user_id = ?',
		userId,
		(err, results) => {
			if (err) {
				console.error(err)
				return res
					.status(500)
					.json({ error: 'Ошибка при получении информации о лицензии' })
			}
...
app.post('/verifyEmail', async (req, res) => {
	...
	try {
		...
		// Проверка, что код подтверждения действителен, прежде чем удалять его
		const validVerification = await db.query(
			'SELECT * FROM email_verification WHERE email = ? AND code = ?',
			[email, code]
		)
		if (validVerification.length === 0) {
			return res.status(400).json({ error: 'Неверный код подтверждения' })
		}
